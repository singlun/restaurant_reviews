...
resetRestaurants = (restaurants) => {
  ...
  const sectionLists = document.getElementById('section-restaurant-lists');
  self.restaurants = [];
  ...
  if (restaurants.length > 0) {
    ...
    sectionLists.appendChild(ul);
  }
  else {
    const noRecElement = document.createElement('h1');
    noRecElement.className = "restaurant-no-records";
    noRecElement.textContent="No Records";
    noRecElement.setAttribute("tabindex", "0");
    sectionLists.appendChild(noRecElement);
...
createRestaurantHTML = (restaurant, index) => {
  ...
  image.setAttribute("alt", DBHelper.imageAltText(index));
  li.append(image);


  const neighborhood = document.createElement('p');
  neighborhood.innerHTML = "Restaurant Address: " + restaurant.neighborhood;
